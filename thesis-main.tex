...
\begin{itemize}
    ...
    \textit{ScaFi} also includes a simulation environment with a \ac{GUI}, integrated with the Alchemist Simulator \cite{Pianini_2013}, an open-source software for simulating and experimenting with distributed systems.
...
The application is implemented using \ac{KMP} and Compose Multiplatform, ensuring \textbf{cross-platform} compatibility between Android and iOS devices.
...
\begin{itemize}
    ...
    \item \textbf{ViewModel}: Acts as an intermediary between the Model and the View. It retrieves and processes data from the Model, exposing it in a \ac{UI}-friendly form. The ViewModel maintains the application state and employs observable data structures to notify the View of any updates.
\end{itemize}
...
\textbf{Collektive Engine.} This layer acts as the Aggregate Computing runtime environment responsible for managing and executing aggregate programs, including the messaging algorithm.
...
\lstinputlisting[language=Kotlin, label={lst:gps-data-code}, caption={Data class for GPS location.}]{listings/DeviceLocation.kt}
...
This layer uses the \ac{MQTT} protocol as its communication mechanism, relying on the public broker \textit{test.mosquitto.org} over WebSocket on port 8080 instead of the default TCP port 1883.
...
\begin{itemize}
    ...
    \item \textbf{Message delivery}: Each device subscribes to its dedicated topic \texttt{echo/messages/<deviceId>} to receive messages.
...
In this project, \textbf{GitHub Actions} is used as the \ac{CI} platform to automate the build processes for both the shared codebase \footnote{\label{collektive-example-repo}\url{https://github.com/Collektive/collektive-examples}} and the mobile application.
...
\lstinputlisting[float, language=yaml, label={lst:CI-collektive-examples}, caption={CI workflow for Collektive examples.}]{listings/CI-collektive-examples.yml}
...
Deploing and testing the messaging system in a real-world environment with many mobile devices can be logistically challenging and require significant resources.
...
\subsection{Simulation with GPS traces}
\texttt{Alchemist} is equipped with the ability to load and simulate on real-world maps. This feature allows for more realistic simulations by incorporating actual geographic data and node placements.

The data of Vienna's 2021 marathon event was used to create a realistic challenging environment to test the messaging system \Cref{fig:marathon-simulation}.
\begin{figure}[h!]
    \centering
