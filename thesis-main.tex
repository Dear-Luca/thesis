...
In the implementation presented here, the loop-avoidance check scans the neighbor’s path prefix, adding an 
...
In sparse geometric graphs ($m = O(n)$) and diameter $D \ll n$, 
typical convergence is closer to $O(snD)$ (pure) or $O(sn^2D)$ (with path check). 
