...
The main features of the framework can be summarized as follows \cite{collektive_docs}:
\begin{itemize}
...
\item \textbf{Dynamic Adaptation:} Applications built with Collektive can dynamically respond to network changes and external stimuli, ensuring robustness and resilience in highly variable environments.
\end{itemize}
...
It leverages \textit{XC}, an experimental programming language designed for the development of homogeneous distributed systems. 
...
The second component is the \textbf{compiler plugin}, which is divided into two submodules: a frontend component that performs static analysis to detect potentially unsafe or inappropriate
 ...
 Each time a function is invoked, its name is recorded within a dedicated data structure to maintain proper execution alignment.
...
The third component, \textbf{Alchemist-incarnation}, enables the integration of Collektive with the \textit{Alchemist Simulator}  \cite{Pianini_2013},
 an open-source environment for modeling and experimenting with distributed and pervasive systems. 
...
\lstinputlisting[language=yaml, label={lst:alchemist-simulation-yaml}, caption={YAML configuration file for the Alchemist simulation.}]{listings/alchemist-simulation.yml}
...
\lstinputlisting[language=Kotlin, label={lst:simulation-entry-point}, caption={Entry point for the multi-source chat simulation.}]{listings/simulationEntryPoint.kt}
