...
Formally, the diffusion speed $T(\delta, n, \epsilon)$
is defined as the smallest number of rounds required for all nodes’ estimates to deviate from the global average by at most a relative error $\epsilon$, with probability at least $1 - \delta$.
...
\[
O(\log n + \log(1/\epsilon) + \log(1/\delta)),
\]
...
To spread the message across the network is used the Collektive's function \texttt{share}, 
which implements efficient stateful data sharing among neighbors.
Every round of the algorithm, the node reads the gossip messages that its neighbors are sharing with him (\texttt{neighborsGossip}),
based on these information it updates its local state.
For each neighbor $v \in \texttt{neighbors(u)} $ where $u$ is the current node, is applied the relaxation:
$$ \boldsymbol{d_{u}^{k+1} \leftarrow  \min(d_{u}^{k+1}, d_{v}^{k} + w(v, u))} $$
...
\section{Multiplatform Mobile Application}
