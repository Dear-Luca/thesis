...
\begin{itemize}
    ...
    \item \textbf{\ac{QoS} 2 – Exactly once:} guarantees that each message is received only once by the intended recipient, at the cost of additional network overhead using using a four-step handshake (PUBLISH, PUBREC, PUBREL, PUBCOMP).
\end{itemize}
...
Future developments may involve replacing \ac{MQTT} with peer-to-peer (\ac{P2P}) communication technologies, such as Bluetooth, to achieve full decentralization while maintaining the same messaging semantics.
...
The mobile application serves as the \ac{GUI} for the messaging system, providing users both an interface and a runtime environment for the algorithm.

\textbf{Echo} is the name chosen for the application, it recalls the idea of messages reverberating through a network within a certain range and time frame (like voice echoing).
It's also the Linux command for printing messages to the terminal, symbolizing communication.

The mobile application is implemented using \ac{KMP} and Compose Multiplatform, ensuring \textbf{cross-platform} compatibility between Android and iOS devices.
In this section the architecture and the implementation details of the app will be described.


\subsection{Architecture}

The architecture of the Echo application is designed to be modular and scalable, allowing for easy maintenance and future enhancements.
It follows a \textbf{\ac{MVVM}} software architectural pattern, separating the user interface, business logic, and data management into distinct layers (\Cref{fig:app-MVVM}).
\begin{figure}
    \centering
    \fbox{\includegraphics[width=.8\linewidth]{figures/mvvm.png}}
    \caption{Illustration of the MVVM architecture pattern.}
    \label{fig:app-MVVM}
\end{figure}

\ac{MVVM} is structured into three main components:
\begin{itemize}
    \item \textbf{Model}: Represents the data and business logic of the application. It is responsible for managing the data, including retrieving it from a database or an API, and performing any necessary transformations or calculations (in this case Aggregate Computing logic).
    \item \textbf{View}: Represents the \ac{UI} of the application, what the user sees and interacts with. It displays the data provided by the ViewModel and sends user interactions (such as button clicks) back to the ViewModel.
    \item \textbf{ViewModel}: Acts as an intermediary between the Model and the View. It retrieves data from the Model, applies any necessary transformations, and exposes the data to the View in a \ac{UI} friendly way. 
    The ViewModel holds the \ac{UI} state and uses observable data structures to notify the View when updates occur.
\end{itemize}

\subsection{Layered design}
The layered design of the application can be seen in \Cref{fig:app-layered-design}.
Now it will be described each layer in a bottom-up approach.

\textbf{Collektive Engine} layer is the Aggregate Computing runtime environment for managing and executing aggregate programs like the messaging algorithm.
It provides primitives for defining fields, sharing data among neighbors, and performing computations over the network like \texttt{share}, \texttt{alignedOn}, \texttt{neighboring} (\Cref{lst:collektive-code}).
\begin{figure}
    \centering
...
\lstinputlisting[language=Kotlin,label={lst:collektive-code}, caption={Collektive engine setup}]{listings/Collektive.kt}

GPS data (\Cref{lst:gps-data-code}) is consumed locally, provided by the \textbf{Location Services} layer, which interfaces with the device's native location APIs to obtain accurate positional information (it's platform specific).
...
GPS data is shared via transport layer for neighbor awareness but it's independent from it.

Location Services layer captures the device's latitude and longitude, essential for computing distances between nodes in the messaging algorithm, and updates location in real time.
To calculate the distance between two devices, given their GPS coordinates, the Haversine formula is used:
\begin{equation}
d = 2r \cdot \operatorname{arcsin}(\sqrt{\sin^2(\frac{\Delta\phi}{2}) + \cos(\phi_1)\cos(\phi_2)\sin^2(\frac{\Delta\lambda}{2})})
\end{equation}

These informations are spread to other nodes through \textbf{Transport Layer}, using \ac{MQTT} protocol as communication mechanism.
The host used for this purpose is the public broker \textit{test.mosquitto.org} over WebSocket with the port 8080 instead of the default 1883 (over TCP).
This is firewall friendly, many private or public networks block non standard ports like 1883, while 8080 is commonly allowed for web traffic.
It also ensures compatibility with browsers and WebApps, allowing future extensions of the application to web platforms.

Transport layer is responsible for:
\begin{itemize}
    \item \textbf{Neighbor discovery}: Every device periodically broadcast its presence via heartbeat (it means that the device is connected to the network) to a wildcard topic
     (will match all heartbeat messages published by any device) \texttt{echo/heartbeat/+} (\Cref{lst:heartbeat-code}).
     \lstinputlisting[float, language=Kotlin,label={lst:heartbeat-code}, caption={Heartbeat pulse recursive sending function.}]{listings/heartBeat.kt}
     The \ac{QoS} used is level 0 (fire and forget), since heartbeats are ephemeral and frequent, this ensures minimal overhead.
    It's also implemented a timeout mechanism to remove neighbors (\Cref{lst:cleanHeartBeat-code}): If a device stops sending heartbeats (crashed, lost connection, moved away) after 5 seconds (\texttt{retentionTime}), it's removed from the neighbor list.
    \lstinputlisting[language=Kotlin,label={lst:cleanHeartBeat-code}, caption={Heartbeat pulse cleanup function.}]{listings/cleanHeartBeat.kt}

    \item \textbf{Message delivery}: 
    
    \item \textbf{Location sharing}
\end{itemize}
