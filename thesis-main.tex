...
To my family, for always supporting me in all my choices.

To my dad, for always being there and helping me through the years.

To my mom, who watches over and guides me from above.
...
\texttt{share} broadcasts the same result to all the neighbors, capturing the space-time evolution of collective computations, allowing the emergence of global behaviors from local updates.
...
When executing the algorithm for \textit{multiple message sources}, 
it is essential to ensure that each computation remains independent, avoiding interference between gradients originating from different nodes. 
Without proper separation, values shared among neighbors could overlap across distinct gradient evaluations, leading to incorrect results and inconsistent field propagation. 
To address this issue, the \texttt{alignedOn} construct is employed. 
This operator provides a mechanism for \textbf{computational alignment}, ensuring that only devices executing the same \texttt{alignedOn(sourceId)} instance exchange information with one another. 
In practice, each source's gossip diffusion is treated as a distinct logical computation with its own stable alignment key.
Without it, multi-source gossip becomes susceptible to state interference and inaccurate distance calculations.
...
The engine handles message alignment, synchronization, and data propagation, ensuring the consistency of aggregate computations across all nodes.
By abstracting the low-level communication details, the \textit{Collektive Engine} allows developers to focus on the logical specification of the distributed behavior.
...
It implements the decentralized proximity-based gossip algorithm responsible for message propagation and handles all user-facing features, including message creation, parameter configuration (\ac{TTL} and maximum propagation distance), and real-time visualization of messages.
...
The \ac{CI} workflow includes the following jobs:
...
To overcome these challenges, the \textit{Alchemist Simulator} is employed to reproduce realistic scenarios in a controlled and reproducible setting. Thanks to the \texttt{Alchemist-incarnation} component integrated within \textit{Collektive}, it is possible to simulate networks composed of many nodes and to analyze the system's behavior under diverse conditions. 
This approach enables the evaluation of scalability, robustness, and communication dynamics without the practical constraints of large-scale physical testing.
...
The left panel depicts the entire network topology, including all nodes and
 ...
 \section{Results}
 The validation phase aimed to confirm the correctness, scalability, and resilience of the proposed messaging system under realistic conditions. 
 The use of the Alchemist Simulator, integrated with Collektive, allowed extensive experimentation in controlled yet dynamic environments, reproducing both synthetic and real-world mobility traces. 
 The large-scale simulation involving approximately $1500$ mobile nodes based on the \textit{Vienna Marathon 2021} GPS data demonstrated that the gossip-gradient algorithm successfully propagated messages across a highly mobile network, 
 maintaining stable diffusion patterns and consistent field formation even under continuous topological changes and challenging conditions. 
 The system exhibited self-stabilizing behavior, with nodes autonomously recovering from disconnections and adapting to varying densities without centralized coordination. 
 Moreover, the decentralized nature of the aggregate model ensured scalability, as communication and computation relied solely on local interactions. 
 These results confirm that the designed solution is robust, adaptive, and scalable, validating the effectiveness of Aggregate Computing as a paradigm for building decentralized, proximity-based messaging systems.
