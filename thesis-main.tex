...
\lstinputlisting[language=Kotlin,label={lst:algorithm-code}, caption={Entire algorithm code}]{listings/algorithm.kt}
...
\lstinputlisting[language=Kotlin,label={lst:gossip-gradient-dataclass-code}, caption={Data structure representing a state in a gossip-based gradient algorithm.}]{listings/GossipGradient.kt}
...
GPS data is consumed locally, provided by the \textbf{Location Services} layer, which interfaces with the device's native location APIs to obtain accurate positional information (it's platform specific).
GPS data is shared via transport for neighbor awareness but it's independent from it.
This layer captures the device's latitude and longitude, which are essential for computing distances between nodes in the messaging algorithm, and updating location in real time.
